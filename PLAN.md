...
- PR2 (완료): 단계 7 — 로컬 웹 앱 온보딩 플로우 + SQLite + provider 설정/연결 테스트 +
  프로젝트 CRUD + 실행.
- PR3 (완료): 단계 5 — 페르소나 자동 추천기(엣지케이스 포함) + 프로젝트 생성 통합.
- PR4 (완료): 단계 6 — 병렬 오케스트레이터(Semaphore + gather, 세션 진행 이벤트).
- PR5 (완료): 단계 8 — 리포트: 버그 집계 클러스터링 + Markdown/HTML 대시보드.
- PR6 (완료): 실시간 진행(SSE) — 백그라운드 실행 + 진행 스트리밍.
- PR7 (현재): 테스트 보강 — 실브라우저 e2e(`-m e2e`), 실 LLM provider mock,
  runner 글루, CI 워크플로(hermetic + e2e 분리). 커버리지 73%→81%(+e2e).
